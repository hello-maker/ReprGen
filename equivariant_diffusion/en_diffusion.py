...
class EnHierarchicalVAE(torch.nn.Module):
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, context, rep=None):
        ...
        # Encoder output.
        z_x_mu, z_x_sigma, z_h_mu, z_h_sigma = self.encode(x, h, node_mask, edge_mask, context, rep=rep)
        ...
        x_recon, h_recon = self.decoder._forward(z_xh, node_mask, edge_mask, context, rep=rep)
        ...
        return loss, {'loss_t': loss.squeeze(), 'rec_error': loss_recon.squeeze()}, xh_rec
    ...
    def forward(self, x, h, node_mask=None, edge_mask=None, context=None, rep=None):
        ...
        loss, loss_dict, xh_rec = self.compute_loss(x, h, node_mask, edge_mask, context, rep=rep)

        neg_log_pxh = loss

        if self.training:
            return neg_log_pxh, xh_rec
        else:
            return neg_log_pxh
    ...
    def encode(self, x, h, node_mask=None, edge_mask=None, context=None, rep=None):
        """Computes q(z|x)."""
        ...
        # Encoder output.
        z_x_mu, z_x_sigma, z_h_mu, z_h_sigma = self.encoder._forward(xh, node_mask, edge_mask, context, rep=rep)
    ...
    def decode(self, z_xh, node_mask=None, edge_mask=None, context=None, rep=None):
        """Computes p(x|z)."""
...
class EnLatentDiffusion(EnVariationalDiffusion):
    ...
    def log_constants_p_h_given_z0(self, h, node_mask, rep=None):
        """Computes p(h|z0)."""
        batch_size = h.size(0)
    ...
    def forward(self, x, h, node_mask=None, edge_mask=None, context=None, rep=None):
        ...
        z_x_mu, z_x_sigma, z_h_mu, z_h_sigma = self.vae.encode(x, h, node_mask, edge_mask, context, rep=rep)
        ...
        # z_xh = z_xh_mean
        z_xh = z_xh.detach()  # NOTE: Trainable in diffusion process only for decoder! Always keep the encoder fixed.
        diffusion_utils.assert_correctly_masked(z_xh, node_mask)
        ...
        if self.trainable_ae: 
            xh = torch.cat([x, h['categorical'], h['integer']], dim=2)
            # Decoder output (reconstruction).
            x_recon, h_recon = self.vae.decoder._forward(z_xh, node_mask, edge_mask, context, rep=rep)
            xh_rec = torch.cat([x_recon, h_recon], dim=2)
        ...
        z_h = {'categorical': torch.zeros(0).to(z_h), 'integer': z_h} # No categorial feature any more.

        if self.training:
            # Only 1 forward pass when t0_always is False.
            loss_ld, loss_dict, denoised_xh = self.compute_loss(z_x, z_h, node_mask, edge_mask, context, t0_always=False, rep=rep)
        else:
            # Less variance in the estimator, costs two forward passes.
            loss_ld, loss_dict, denoised_xh = self.compute_loss(z_x, z_h, node_mask, edge_mask, context, t0_always=True, rep=rep)
    ...
    def sample(self, n_samples, n_nodes, node_mask, edge_mask, context, fix_noise=False, rep=None):
        ...
        Draw samples from the generative model.
        """
        z_x, z_h = super().sample(n_samples, n_nodes, node_mask, edge_mask, context, fix_noise, rep=rep)

        z_xh = torch.cat([z_x, z_h['categorical'], z_h['integer']], dim=2)
        diffusion_utils.assert_correctly_masked(z_xh, node_mask)
        x, h = self.vae.decode(z_xh, node_mask, edge_mask, context, rep=rep)
    ...
    def sample_chain(self, n_samples, n_nodes, node_mask, edge_mask, context, keep_frames=None, rep=None):
        ...
        Draw samples from the generative model, keep the intermediate states for visualization purposes.
        """
        chain_flat = super().sample_chain(n_samples, n_nodes, node_mask, edge_mask, context, keep_frames, rep=rep)
